...
let FinanceService = class FinanceService {
    ...
    async getStats(tenantId) {
        ...
        const calculateBalance = async (ids) => {
            if (ids.length === 0)
                return 0;
    ...
    async getChartData(tenantId, period = 'monthly') {
        let startDate;
        ...
        payments.forEach(p => {
            ...
            let pointIndex = -1;
            if (interval === 'month') {
                pointIndex = dataPoints.findIndex(m => m.date.getMonth() === pDate.getMonth() &&
                    m.date.getFullYear() === pDate.getFullYear());
            }
            else if (interval === 'day') {
                pointIndex = dataPoints.findIndex(m => m.date.getDate() === pDate.getDate() &&
            ...
            if (pointIndex !== -1) {
                if (p.studentId)
                    dataPoints[pointIndex].income += Number(p.amount);
                if (p.expenseId)
                    dataPoints[pointIndex].expense += Number(p.amount);
            }
        });
    ...
    async getRecentStudentPayments(tenantId, limit = 5) {
        return this.prisma.payment.findMany({
            where: {
                studentId: { not: null },
                tenantId, // Enforce tenant
    ...
    async getRecentExpenses(tenantId, limit = 5) {
