...
let CompteService = class CompteService {
    ...
    async create(tenantId, createCompteDto) {
        let { name, code, nameAr, parentId, employerId, studentId, isPosted, category, nature, selectionCode, isFeeCash, showInParent } = createCompteDto;
        parentId = parentId ?? -1;
    ...
    async findAll(tenantId, page = 1, limit = 10, search, status) {
        ...
        const comptesWithBalance = comptes.map((compte) => {
    ...
    async update(tenantId, id, updateDto) {
        const { name, code, nameAr, parentId: newParentIdInput, isPosted, employerId, studentId, category, nature, isFeeCash, showInParent, selectionCode } = updateDto;
    ...
    async getTransactions(tenantId, id, startDate, endDate, page = 1, limit = 20, search) {
        const skip = (page - 1) * limit;
        const where = { compteId: id, tenantId }; // Enforce tenant
        const dateFilter = {};
        if (startDate)
    ...
    async createTransaction(tenantId, compteId, dto, userId) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (mainAccount?.category === 'CAISSE')
                journalId = 2;
            if (mainAccount?.category === 'BANQUE')
                journalId = 3;
    ...
    async updateTransaction(tenantId, entryId, dto, userId) {
        const { amount, type, description, contraAccountId, date } = dto;
        const transactionDate = date ? new Date(date) : new Date();
    ...
    async deleteTransaction(tenantId, entryId) {
    ...
    async ensureSalaryParentAccount(tenantId) {
    ...
    async getOrCreateEmployerAccount(tenantId, employerId, name) {
