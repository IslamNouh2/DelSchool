...
export class AuthService {
    ...
    async register(registerDto: RegisterDto, response: Response) {
        ...
        const payload: JwtPayload = { sub: user.id, username: user.username, email: user.email, role, permissions, tenantId: user.tenantId };
        ...
        return { user: { ...user, role, permissions }, message: 'Registration successful' };
    }
    ...
    async getProfile(userId: number) {
        ...
        if (roleName === 'STUDENT') {
            const student = await this.prisma.student.findFirst({
                where: { code: user.username, tenantId: user.tenantId as string }
            });
            profileId = student?.studentId || null;
        } else if (roleName === 'TEACHER') {
            const employer = await this.prisma.employer.findFirst({
                where: { code: user.username, tenantId: user.tenantId as string, type: 'teacher' }
            });
