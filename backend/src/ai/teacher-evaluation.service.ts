...
export class TeacherEvaluationService {
  ...
  private async buildFeatures(teacher: any): Promise<number[]> {
    ...
    const avgAttendance = studentCount > 0 ? totalAttendanceRatio / studentCount : 0.9;
    const avgBehavior = studentCount > 0 ? totalBehaviorScore / studentCount : 80;
    const avgHomework = studentCount > 0 ? totalHomeworkCompletion / studentCount : 0.8;
  ...
  private async callAiService(features: number[]) {
    try {
      ...
      throw new InternalServerErrorException(`Failed to connect to AI engine: ${error.message}`);
    }
  }
