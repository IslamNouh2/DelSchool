...
export class SubjectService {
  ...
  async create(tenantId: string, createSubjectDto: CreateSubjectDto) {
    const { subjectName, totalGrads, coff } = createSubjectDto;
    ...
    const { BD: parentBD, level: parentLevel } = parent || {
      BG: 0,
      BD: 0,
      level: -1,
    };
      ...
      const newSubject = await tx.subject.create({
        data: {
          ...
          translations: createSubjectDto.translations
            ? {
                create: Object.entries(createSubjectDto.translations).map(
                  ([locale, name]) => ({
                    locale,
                    name,
                  }),
  ...
  async update(
    ...
    const parentId = param?.okActive === false ? -1 : (newParentIdInput ?? -1);
      ...
      await this.prisma.subject.update({
        ...
        data: {
          ...
          translations: updateDto.translations
            ? {
                deleteMany: {},
                create: Object.entries(updateDto.translations).map(
                  ([locale, name]) => ({
                    locale,
                    name,
                  }),
