  ...
  async createEmployer(
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw new BadRequestException(
            'Ce code existe déjà. Veuillez en choisir un autre.',
          );
        }
      }
    ...
    try {
      const updated = await this.prisma.employer.update({
        ...
        data: {
          ...
          type: dto.type,
          weeklyWorkload:
            dto.weeklyWorkload !== undefined ? dto.weeklyWorkload : undefined,
  ...
  async assignClassToTeacher(
    ...
    if (!isSubjectBased) {
      ...
      if (classData?.local?.subject_local?.length > 0) {
        const subjectIds = classData.local.subject_local.map(
          (sl) => sl.subjectId,
        );
        ...
        console.log(
          `[DualSystem] Assigned ${newAssignments.length} subjects automatically to teacher ${employerId}`,
        );
      }
    }
