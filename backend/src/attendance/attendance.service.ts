...
export class AttendanceService {
  ...
  async updateEmployer(
      ...
      if (eid) {
        ...
        if (employer?.checkInTime) {
          ...
          const globalThreshold =
            await this.parameterService.getLateThreshold();
          thresholdHours = globalThreshold.hours;
          thresholdMinutes = globalThreshold.minutes;
        }
    ...
    const result = await this.repo.updateEmployer(tenantId, id, {
      ...dto,
      status,
    });
    this.socketGateway.emitRefresh();
  ...
  async getEmployerStats(tenantId: string, employerId: number) {
    const attendance = await this.repo.getEmployerAttendanceByEmployerId(
      tenantId,
      employerId,
    );
