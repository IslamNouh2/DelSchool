...
export default function StudentFinancialDashboard() {
    ...
    const handleNewFee = async (data: any) => {
        ...
        try {
            ...
            toast.success("Frais assigné avec succès");
            fetchData();
        } catch (error: any) {
    ...
    const handleBulkSubscribe = async (data: { templateId: number, dueDate: string }) => {
        try {
            ...
            toast.success("Abonnement groupé réussi");
            fetchData();
        } catch (error: any) {
            toast.error(error.response?.data?.message || "Erreur lors de l'abonnement groupé");
        }
    };
        ...
        <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 p-8 space-y-10 animate-in fade-in duration-700">
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-4xl font-[1000] tracking-tighter text-gray-900 dark:text-gray-100 uppercase">
                            Gestion <span className="text-blue-600 dark:text-blue-400">Financière</span>
                        </h1>
                    </div>
                    <p className="text-gray-500 dark:text-slate-400 font-bold text-sm uppercase tracking-widest flex items-center gap-2">
                        Administration du flux de trésorerie scolaire
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className={cn(
                        ...
                        <span className="text-xs font-black uppercase tracking-widest">
                            Système: {isSystemPaid ? "Mode Payant Activé" : "École Gratuite"}
                        </span>
                    </div>
                    ...
                    <Button 
                        ...
                        variant="outline" 
                        className="rounded-2xl h-12 px-6 border-gray-200 dark:border-slate-800 text-gray-600 dark:text-slate-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-700 dark:hover:text-amber-400 hover:border-amber-100 dark:hover:border-amber-900/30 transition-all font-black text-xs uppercase tracking-widest gap-2 shadow-sm bg-white dark:bg-slate-900"
                    >
                        <Users size={18} />
                        Abonnement Groupé
                    </Button>
                    ...
                    <Button 
                        ...
                        variant="outline"
                        className="rounded-2xl h-12 px-6 border-gray-200 dark:border-slate-800 text-gray-600 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-700 dark:hover:text-blue-400 hover:border-blue-100 dark:hover:border-blue-900/30 transition-all font-black text-xs uppercase tracking-widest gap-2 shadow-sm bg-white dark:bg-slate-900"
                    >
                        <Plus size={18} />
                        Nouveau Frais
                    </Button>
                    ...
                    <Button 
                        ...
                        className="rounded-2xl h-12 px-8 bg-gray-900 hover:bg-black dark:bg-blue-600 dark:hover:bg-blue-500 text-white transition-all font-black text-xs uppercase tracking-widest gap-2 shadow-xl shadow-gray-900/10 dark:shadow-blue-900/30 active:scale-95 border-none"
                    >
                        <CreditCard size={18} />
                        Collecter Paiement
                    </Button>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800">
                <div className="flex items-center gap-4 w-full md:w-auto flex-1">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <Input 
                            ...
                            className="pl-9 rounded-xl border-gray-200 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-950/50 focus:bg-white dark:focus:bg-slate-900 transition-all font-medium"
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                    ...
                    <div className="flex items-center gap-2 min-w-fit">
                         ...
                         <span className="text-xs font-black uppercase tracking-widest text-gray-500">Filtres</span>
                    </div>
                    
                    <Select value={classId} onValueChange={(v) => { setClassId(v); setPage(1); }}>
                        <SelectTrigger className="w-[180px] rounded-xl border-gray-200 dark:border-slate-800 font-bold bg-gray-50/50 dark:bg-slate-950">
                            <SelectValue placeholder="Toutes les classes" />
                        </SelectTrigger>
                        <SelectContent className="rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                            <SelectItem value="all" className="font-bold cursor-pointer">Toutes les classes</SelectItem>
                            {classes.map((cls: any) => (
                                <SelectItem key={cls.id || cls.classId} value={String(cls.id || cls.classId)} className="font-bold cursor-pointer">
                    ...
                    <Select value={status} onValueChange={(v) => { setStatus(v); setPage(1); }}>
                        ...
                        <SelectContent className="rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                            <SelectItem value="ALL" className="font-bold cursor-pointer">Tous les status</SelectItem>
                            <SelectItem value="PAID" className="font-bold cursor-pointer text-emerald-600">Payé</SelectItem>
                            <SelectItem value="PARTIAL" className="font-bold cursor-pointer text-amber-600">Partiel</SelectItem>
                            <SelectItem value="UNPAID" className="font-bold cursor-pointer text-blue-600">À Venir / Non Payé</SelectItem>
                             <SelectItem value="OVERDUE" className="font-bold cursor-pointer text-rose-600">En Retard</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
                ...
                {isLoading && (
                    <div className="absolute inset-0 bg-white/20 dark:bg-slate-950/40 backdrop-blur-[2px] rounded-[2rem] flex items-center justify-center z-50 transition-all">
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-2xl border border-gray-50 dark:border-slate-800 flex flex-col items-center gap-4">
                            ...
                            <p className="text-[10px] font-black uppercase text-gray-400 dark:text-slate-500 tracking-[0.2em]">Chargement des données...</p>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center p-4 bg-white dark:bg-slate-900 rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-sm">
                 <div className="text-gray-500 text-sm font-medium">
                    Affichage de <span className="font-bold text-gray-900 dark:text-gray-100">{students.length}</span> sur <span className="font-bold text-gray-900 dark:text-gray-100">{totalPages * limit}</span> résultats estimés
                 </div>
