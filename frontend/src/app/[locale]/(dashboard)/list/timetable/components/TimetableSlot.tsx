    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-2">
                <p className={`text-sm font-medium ${isLunch ? 'text-muted-foreground' : 'text-foreground'}`}>{slot.subject?.subjectName || t("unknown_subject")}</p>
            ...
            {!isLunch && (
                <>
                    ...
                    <p className="text-xs text-muted-foreground/80 lowercase first-letter:uppercase">
                        {slot.class?.ClassName || ''} {slot.room ? `• ${t("room", { room: slot.room })}` : ''}
                    </p>
                </>
            )}
        </div>
