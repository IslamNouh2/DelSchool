...
export default function PayrollSettings() {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-5">
            <Card className="border-none shadow-2xl rounded-[3rem] bg-white/50 backdrop-blur-sm overflow-hidden">
                ...
                <CardContent className="p-10 space-y-8">
                    <div className="grid md:grid-cols-2 gap-8">
                        {/* Late Threshold */}
                        <div className="space-y-4 p-6 rounded-[2rem] bg-amber-50/50 border border-amber-100">
                            <div className="flex items-center gap-3 mb-2">
                                <Clock className="w-6 h-6 text-amber-600" />
                                <Label className="text-lg font-black uppercase text-amber-900 tracking-tighter italic">Seuil de Retard</Label>
                            </div>
                            <Input
                                type="time"
                                value={settings.Attendance_Late_Threshold}
                                onChange={(e) => setSettings({ ...settings, Attendance_Late_Threshold: e.target.value })}
                                className="h-14 rounded-2xl border-amber-200 bg-white font-black text-lg focus:ring-amber-500 shadow-sm"
                            />
                            <p className="text-[10px] font-bold text-amber-700 uppercase tracking-widest px-2">
                                Les arrivées après cette heure sont marquées comme "En Retard".
                            </p>
                        </div>

                        {/* Monthly Days */}
                        <div className="space-y-4 p-6 rounded-[2rem] bg-blue-50/50 border border-blue-100">
                            <div className="flex items-center gap-3 mb-2">
                                <Calculator className="w-6 h-6 text-blue-600" />
                                <Label className="text-lg font-black uppercase text-blue-900 tracking-tighter italic">Base de Jours (Mensuel)</Label>
                            </div>
                            <Input
                                type="number"
                                value={settings.Payroll_Monthly_Days}
                                onChange={(e) => setSettings({ ...settings, Payroll_Monthly_Days: e.target.value })}
                                className="h-14 rounded-2xl border-blue-200 bg-white font-black text-lg focus:ring-blue-500 shadow-sm"
                            />
                            <p className="text-[10px] font-bold text-blue-700 uppercase tracking-widest px-2">
                                Diviseur utilisé pour calculer le taux journalier (Salaire / X).
                            </p>
                        </div>

                        {/* Penalty Ratio */}
                        <div className="space-y-4 p-6 rounded-[2rem] bg-rose-50/50 border border-rose-100 md:col-span-2">
                            <div className="flex items-center gap-3 mb-2">
                                <Percent className="w-6 h-6 text-rose-600" />
                                <Label className="text-lg font-black uppercase text-rose-900 tracking-tighter italic">Ratio de Pénalité Retard</Label>
                            </div>
                            <div className="flex items-center gap-4">
                                <Input
                                    ...
                                    className="h-14 w-32 rounded-2xl border-rose-200 bg-white font-black text-lg focus:ring-rose-500 shadow-sm"
                                />
                                <div className="text-sm font-black text-rose-800 uppercase tracking-tighter italic">
                                    {settings.Payroll_Late_Penalty_Ratio} Retards = 1 Jour de Déduction
                                </div>
                            </div>
                            <p className="text-[10px] font-bold text-rose-700 uppercase tracking-widest px-2">
